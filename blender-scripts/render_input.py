...
def render_category(pcd_path: Path, image_path: Path, modifier: GeometryNodeTree) -> None:
    ...
    full_IDs = [ply_file.stem for ply_file in list(pcd_path.glob('*.ply'))]
    for full_ID in full_IDs:
        ply_file = pcd_path / 'test' / f'{full_ID}.ply'
        try:
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Render input pointclouds. Each point is rendered as a sphere')
    parser.add_argument('-r', '--radius', type=float, default=0.005, help='Radius of each point')
    parser.add_argument('pcd_dir', type=str, help='Path to the point cloud dataset')
    ...
    modifier = prepare.create_pointcloud_modifier(material)
